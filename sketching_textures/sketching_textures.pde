...
void setup(){
  ...
  wall.setPosition(worldWidth/2.0-5, worldHeight/2.0-4);
  wall.setDensity(100)                                 ;
  ...
  area1.setPosition(worldWidth/2.0-6, worldHeight/2.0+3);
  area1.setDensity(0)                                   ;
  ...
  area2.setPosition(worldWidth/2.0+2, worldHeight/2.0+3);
  area2.setDensity(0)                                   ;
  ...
  area3.setPosition(worldWidth/2.0+10, worldHeight/2.0+3);
  area3.setDensity(0)                                    ;
  ...
  area4.setPosition(worldWidth/2.0+7, worldHeight/2.0-3.5);
  area4.setDensity(0)                                   ;
...
class SimulationThread implements Runnable{
  ...
  public void run(){
    ...
    if (s.h_avatar.isTouchingBody(area3)) {
      //fEE.x = 1.1*sin((float)(millis())/1000.0 * radpers);
      //fEE.y = 1.1*cos((float)(millis())/1000.0 * radpers);
    ...
    if (s.h_avatar.isTouchingBody(wall) && wall.isSensor()){
      ...
      if (millis() > lasttimecheck1 + timeinterval1){
        ...
        if ((t >= 400) && (t < 500)){
          s.h_avatar.setDamping(random(970,990)) ;
      ...
      if (millis() > lasttimecheck1 + timeinterval1){
        ...
        bumps = new FCircle(0.75) ;
        bumps.setPosition(s.h_avatar.getX(), s.h_avatar.getY()-0.2);
        bumps.setNoFill()         ;
        bumps.setDensity(100)     ;
        bumps.setStroke(0,0,0,30) ;
        bumps.setSensor(false)    ;
        bumps.setStatic(false)    ;
        world.add(bumps)          ;
      }
     
     if((bumps.isTouchingBody(area1)) || (bumps.isTouchingBody(area2)) || (bumps.isTouchingBody(area3))) {
      world.remove(bumps);
     }
